...
(deftest test-app
  (testing "main route"
    (let [response (app (request :get "/"))]
      ...
      (is (= "default-src 'self'; img-src *; frame-src *; child-src *; style-src 'self' 'unsafe-inline'; script-src 'self' www.google-analytics.com platform.twitter.com connect.facebook.net" ((:headers response) "Content-Security-Policy")))
