...
(deftest comment-notifier
  (testing "an HTML template is created with links to the swirl"
    (let [code (UUID/randomUUID)
      ...
      (is (.contains html (str "<a href=\"https://www.swrl.co/swirls/1?code=" (str code) "\">Some &lt;great&gt; thing</a>")) html)
      (is (.contains html (str "F&lt;rank&gt; has recommended that you check out")) html)
      (is (.contains html (str "<a href=\"https://www.swrl.co/notification-options?email=blah%40example.org\">click here</a>")) html))))
